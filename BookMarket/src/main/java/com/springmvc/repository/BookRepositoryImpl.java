...
public class BookRepositoryImpl implements BookRepository {
	private JdbcTemplate template; // JdbcTemplate 클래스를 사용하기 위해 JdbcTemplate 객체 타입의 jdbcTemplate을 선언
...
//		book2.setReleaseDate("2020/07/29");
//		
//		Book book3 = new Book("ISBN1236", "어도비", 25000);
//		book3.setAuthor("김두한");
...
//		book3.setReleaseDate("2019/04/29");
//		
...
//			}
		...
		for(int j = 0; j < filter.get("publisher").size(); j++) {
			String publisherName = filter.get("publisher").get(j);
			String SQL = "SELECT * FROM book WHERE b_publisher LIKE '%" + publisherName + "%'"; // 2
			booksByPublisher.addAll(template.query(SQL, new BookRowMapper())); // 3
		}
	}
	...
	if(criterias.contains("category")) {
		for(int j = 0; j < filter.get("category").size(); j++) {
			String category = filter.get("category").get(j);
			String SQL = "SELECT * FROM book WHERE b_category LIKE '%" + category + "%'"; // 4
			booksByCategory.addAll(template.query(SQL, new BookRowMapper())); // 5
			}
		}
